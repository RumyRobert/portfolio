...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def startBattle(self):
        ...
        global isEXT
        if isEXT: # set game speed to max
            a=0
        else:
            a=(self.horizontalSlider.value()**2)/100.0
        self.timer.start(a)
        self.resizeEvent()
    ...
    def chooseAction(self):
        ...
        if self.countBattle >= self.spinBox.value():
            ...
            for key, value in self.statisticDico.items():
                ...
                #print("points, ",str(key),str(value.points))
                listRET.append(str(value.points)+","+str(key))
                i += 1
